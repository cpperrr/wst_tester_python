...
class WSTCan:
	...
	#  returns current status if no param, otherwise set on or off.
	def bluebotics_protocol_enabled(self, setstatus=None):
		if setstatus is None:
			return self.protocol_2_ids == self.protocol_2_bluebotics_ids
		if setstatus:
			self.set_protocol2_ids(self.protocol_2_bluebotics_ids[0], self.protocol_2_bluebotics_ids[1])
		else:
			self.set_protocol2_ids(self.protocol_2_default_ids[0], self.protocol_2_default_ids[1])
	...
	def isBitSet(self, bitfield, nthBit):
		if bitfield & (1 << (nthBit)):
			return True
		else:
			return False

	# Expects position to be 0 for first bit
	def set_bit(self, bitfield, nthBit):
		return bitfield | 1 << nthBit

	def unset_bit(self, bitfield, nthBit):
		return ~(1 << nthBit) & bitfield
	...
	def init_filters(self, extra_can_filter=[]):
		...
		if result != PCAN_ERROR_OK:
			...
			can_filters = [
				[1, 3],
				[self.protocol_2_ids[1], self.protocol_2_ids[1]],  # We use the configured can_id to receive protocol 2
				[0x7C0, 0x7C0]
			]
	...
	def getStatusCodeDescAbbrArray(self):
		...
		for codeToCheck in _POSSIBLESTATUSCODES:
			...
			if self.isBitSet(byteToCheck, bitLocation):
	...
	def readWSTFrame(self, expectedID="will be overwritten below", timeout=10, sleepTime=0.050, verbose=False,
									 fast=False):
		self.init_filters(extra_can_filter=[expectedID, expectedID])
	...
	def getBMSModelString(self, dataFormat="ascii", verbose=False, retries=10, spnaming=False):
		...
		if spnaming:
			...
			return self.TranslateSPBMSModelToWSTNaming(
				self.getBMSModelStringRecursiveFunction(dataFormat=dataFormat, verbose=verbose, retries=retries))
		#
	...
	def ParseAndSaveProtocol2LogToFile(self, log_array, filename, subdir="."):
		...
		for e, record in enumerate(log_array):
			...
			date_time_str = "20" + self.intToHexString(record[1]) + "-" + self.intToHexString(
				record[2]) + "-" + self.intToHexString(
				record[3]) + " " + self.intToHexString(record[4]) + ":" + self.intToHexString(
				record[5]) + ":" + self.intToHexString(
				record[6])
			try:
	...
	def write_custom_parameter_short_int(self, node_id, parameter_number, data, verbose=False):
		...
		payloadWithOutChecksum = [0xBD, int(node_id), int(data_bytes[0]), int(parameter_number), int(data_bytes[1]), 0x04,
															0x80]
		# print("payload no checksum %s" % payloadWithOutChecksum)
		# calculate checksum and append
		checksum = 0
	...
	def get_parsed_sp_status(self, status_type="realtime"):
		...
		if status_type == "parameters":
			...
			status_data = _sp_parser.parse_sp_parameters(parameter_data, status_data,
	...
	def write_cell_voltage_calibrations(self, calibration_data):
		...
		for cell_number, cell_voltage in calibration_data.items():
			new_coefficient = int(
				self.calculate_cell_voltage_coefficient(raw_cell_voltages[cell_number - 1], round(cell_voltage, 3)))
